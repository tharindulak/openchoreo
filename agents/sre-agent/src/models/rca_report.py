...
class HandoffResult(BaseModel):
    """The persisted record of the handoff stage's last tool call.

    Fully generic on purpose: which tool answered, and what it answered,
    carried verbatim. What any of it MEANS — classification, dedupe, adoption
    — belongs to whatever platform received the call; typing those fields
    here would mean this record could only ever describe one receiver.
    """

    tool: str | None = Field(
        default=None,
        description="Name of the last tool call the stage made, or None if it made none",
    )
    result: Any = Field(
        default=None,
        description="That call's parsed result, carried verbatim and never interpreted here",
    )
    failure_reason: str | None = Field(
        default=None,
        description=(
            "Why the handoff stage failed before it could file, when it did. Set only by "
            "`failed()` — absent on a completed run, because the last tool call itself is "
            "the record of what happened"
        ),
    )

    @classmethod
    def compose(cls, outcome: dict[str, Any] | None) -> "HandoffResult":
        """Assemble the persisted record from the stage's last recorded call.

        `outcome` is `ToolCallRecorder`'s last entry (`{"tool": ..., "result": ...}`),
        or `None` when the stage made no tool call at all. No model input
        reaches this beyond what the model itself caused to be called.
        """
        if outcome is None:
            return cls()
        return cls(tool=outcome.get("tool"), result=outcome.get("result"))
