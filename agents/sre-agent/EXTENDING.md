...
class MyBackend(ReportBackend):
    ...
    async def try_acquire_handoff_slot(self, dedupe_key, cooldown_seconds): ...
```

A custom implementation must also implement `try_acquire_handoff_slot` (added for the handoff
cooldown gate, see `HANDOFF-COOLDOWN-DESIGN.md`) — otherwise it can't be instantiated
