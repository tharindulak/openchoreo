...
var invalidErr = apierrors.NewInvalid(
	schema.GroupKind{Group: "openchoreo.dev", Kind: "ProjectType"},
...
func TestProjectTypeService_ClientErrors(t *testing.T) {
	...
	t.Run("create wraps Invalid as ValidationError", func(t *testing.T) {
		...
		_, err := svc.CreateProjectType(ctx, testNamespace, testutil.NewProjectType(testNamespace, "pt"))
	...
	t.Run("create existence-check error", func(t *testing.T) {
		...
		_, err := svc.CreateProjectType(ctx, testNamespace, testutil.NewProjectType(testNamespace, "pt"))
		require.Error(t, err)
		assert.Contains(t, err.Error(), "check project type existence")
	})

	t.Run("update of missing returns NotFound", func(t *testing.T) {
		svc := newServiceWithInterceptor(t, interceptor.Funcs{})
		_, err := svc.UpdateProjectType(ctx, testNamespace, testutil.NewProjectType(testNamespace, "pt"))
	...
	t.Run("update wraps Invalid as ValidationError", func(t *testing.T) {
		c := fake.NewClientBuilder().
			...
			WithObjects(testutil.NewProjectType(testNamespace, "pt")).
	...
	t.Run("list error wrapped", func(t *testing.T) {
		...
		_, err := svc.ListProjectTypes(ctx, testNamespace, services.ListOptions{})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "failed to list project types")
	})
	...
	t.Run("get error wrapped", func(t *testing.T) {
		...
		_, err := svc.GetProjectType(ctx, testNamespace, "pt")
		require.Error(t, err)
		assert.Contains(t, err.Error(), "failed to get project type")
	})
	...
	t.Run("delete error wrapped", func(t *testing.T) {
		...
		err := svc.DeleteProjectType(ctx, testNamespace, "pt")
		require.Error(t, err)
		assert.Contains(t, err.Error(), "failed to delete project type")
	})

	t.Run("create AlreadyExists on apply", func(t *testing.T) {
		svc := newServiceWithInterceptor(t, interceptor.Funcs{
			Create: func(context.Context, client.WithWatch, client.Object, ...client.CreateOption) error {
				return apierrors.NewAlreadyExists(schema.GroupResource{Group: "openchoreo.dev", Resource: "projecttypes"}, "pt")
			},
		})
		_, err := svc.CreateProjectType(ctx, testNamespace, testutil.NewProjectType(testNamespace, "pt"))
	...
	t.Run("create generic error wrapped", func(t *testing.T) {
		...
		_, err := svc.CreateProjectType(ctx, testNamespace, testutil.NewProjectType(testNamespace, "pt"))
		require.Error(t, err)
		assert.Contains(t, err.Error(), "failed to create project type")
	})
	...
	t.Run("update generic error wrapped", func(t *testing.T) {
		...
		_, err := svc.UpdateProjectType(ctx, testNamespace, testutil.NewProjectType(testNamespace, "pt"))
		require.Error(t, err)
		assert.Contains(t, err.Error(), "failed to update project type")
	})

	t.Run("schema conversion error wrapped", func(t *testing.T) {
		// Inject a retrievable ProjectType whose parameters schema is invalid JSON,
		// so SectionToRawJSONSchema fails (a malformed Raw cannot survive the fake
		// client's own (de)serialization, hence the interceptor).
		svc := newServiceWithInterceptor(t, interceptor.Funcs{
		...
		_, err := svc.GetProjectTypeSchema(ctx, testNamespace, "pt")
		require.Error(t, err)
	...
	t.Run("list surfaces remaining item count", func(t *testing.T) {
		svc := newServiceWithInterceptor(t, interceptor.Funcs{
			List: func(_ context.Context, _ client.WithWatch, obj client.ObjectList, _ ...client.ListOption) error {
				l := obj.(*openchoreov1alpha1.ProjectTypeList)
				remaining := int64(7)
